...
Page({
    ...
    onLoad: function () {
		...
		// MusicService.getBanner(that.initSwiper);//轮播接口
    },
    ...
    radioTap: function (e) {
        ...
        MusicService.getRadioMusicList(dataSet.id, function (data) {
            wx.navigateTo({
                url: '../toplist/toplist?id='+dataSet.id
            });
            if (data.code == 0) {
                var list = [];
    ...
    delHistoryItem: function (e) {//删除历史记录
        var historySearchs = this.data.historySearchs;
        var dataSet = e.currentTarget.dataset;
        if (dataSet.index != '') {
    ...
    addPlayingSongs: function (song) {//添加播放歌曲
        var playingSongs = app.globalData.playList;
        var index = -1;
